// Porting from
// https://github.com/supabase/supabase/blob/master/studio/components/ui/CodeEditor/Providers/PgSQLCompletionProvider.ts

import { RefObject } from 'react'
import BackwardIterator from './backward-iterator'
import { Schema, TableColumn, DatabaseFunction } from '@/types/database'
  ...
  if (pgInfoRef.current.tableColumns?.length > 0) {
    pgInfoRef.current.tableColumns.forEach((x: TableColumn) => {
      ...
      items.push({
        ...
        kind: x.is_table ? monaco.languages.CompletionItemKind.Class : monaco.languages.CompletionItemKind.Interface,
